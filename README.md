...
### Gated model fetching — the supply-chain story (offline)

```bash
rusty demo-hf-gate.lisp
```

The July 2026 registry-breach pattern — an agent whose only security layer was
its model's refusals — restated as a wuwei tool: the agent may *propose* any
download; a byte moves only after the call is proven permitted. Four gates:
a **parsed-host allowlist** (`hub.example@evil.example` is rejected — the host
is parsed, never string-matched), **`.safetensors` only** (a pickle-style
`.bin` never reaches the transport), **pin required** (a URL absent from the
`(url sha256)` lockfile is unfetchable), and **hash verification** (bytes land
in quarantine, and only an exact SHA-256 pin match installs into `models/` —
a tampered artifact is deleted and refused, with both hashes named). The
fetcher itself must pass boot certification: a variant that hides its write is
refused before anything runs. Golden-tested in `hf-gate-test.lisp`.

Narrow claim: pinned, allowlisted, format-gated fetches by an effect-honest
tool. Not claimed: artifact benignity, platform security, or a scanner.
...
| `demo-hf-gate.lisp` | **60s offline gated-fetch story** (no LLM) |
