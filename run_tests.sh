...
run_test battle-test.lisp expected_battle.txt "battle-test.lisp (jailbreak challenge — 13 attacks incl. injection + spec clobber, 0 breaches)"
run_test guards-test.lisp expected_guards.txt "guards-test.lisp (safe-under? closes the symlink AND hardlink escape — needs Rusty ≥0.78.1)"
...
run_test enforced-test.lisp expected_enforced.txt "enforced-test.lisp (FS boundary is the kernel-enforced sandbox — a weak guard still can't escape; needs Rusty ≥0.82.0)"
